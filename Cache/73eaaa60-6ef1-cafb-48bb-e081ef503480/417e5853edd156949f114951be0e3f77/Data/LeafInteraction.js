...
function findInteractableComponent(sceneObject) {
    var components = sceneObject.getComponents("ScriptComponent");
    for (var i = 0; i < components.length; i++) {
        if (components[i].name === "Interactable") {
            return components[i];
        }
    }
    return null;
...
function bindTriggerStart(interactableComponent, callback) {
    if (!interactableComponent || !interactableComponent.onInteractorTriggerStart) {
        return false;
    }
    if (interactableComponent.onInteractorTriggerStart.add) {
        interactableComponent.onInteractorTriggerStart.add(callback);
        return true;
    }
    if (typeof interactableComponent.onInteractorTriggerStart === "function") {
        interactableComponent.onInteractorTriggerStart(callback);
        return true;
    }
    return false;
}
...
script.createEvent("OnStartEvent").bind(function () {
    ...
    // --- Set up SIK Interactable (SIK 2.0 uses .add() events) ---
    try {
        interactable = findInteractableComponent(script.getSceneObject());

        if (interactable && bindTriggerStart(interactable, function () {
            onTriggerStart();
        })) {
