...
export class SphereVisual extends Visual {
  // Per-state base colors now live in Visual._states (set in the constructor before
  // initialize()); the old shadowed flat _*Color fields were removed by the grouping.
  ...
  public constructor(args: VisualArgs) {
    ...
    // Base per-state colors (formerly the shadowed _*Color fields) → grouped storage,
    // before initialize() builds the state map.
    this._states.default.color = SphereColors.default.base
    this._states.hovered.color = SphereColors.hovered.base
    this._states.triggered.color = SphereColors.triggered.base
    this._states.toggledDefault.color = SphereColors.triggered.base
    this._states.toggledHovered.color = SphereColors.toggledHovered.base
    this.initialize()
  }
  ...
  public set defaultIcon(icon: Texture) {
    ...
    if (this._defaultIcon?.isSame(icon)) {
      return
    }
  ...
  public set hoveredIcon(hoveredIcon: Texture) {
    ...
    if (this._hoveredIcon?.isSame(hoveredIcon)) {
      return
    }
  ...
  public set triggeredIcon(icon: Texture) {
    ...
    if (this._triggeredIcon?.isSame(icon)) {
      return
    }
  ...
  public set inactiveIcon(icon: Texture) {
    ...
    if (this._inactiveIcon?.isSame(icon)) {
      return
    }
  ...
  public set errorIcon(icon: Texture) {
    ...
    if (this._errorIcon?.isSame(icon)) {
      return
    }
