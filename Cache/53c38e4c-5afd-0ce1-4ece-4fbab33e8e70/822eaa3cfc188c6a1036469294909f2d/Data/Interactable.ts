...
/**
 * Determines the targeting priority of an Interactable.
 * Higher-priority interactables are preferred over closer, lower-priority ones
...
export class Interactable extends BaseScriptComponent {
  ...
  private onSecondaryTriggerStartEvent = new Event<InteractorEvent>()
  private onSecondaryTriggerUpdateEvent = new Event<InteractorEvent>()
  private onSecondaryTriggerEndEvent = new Event<InteractorEvent>()
  private onSecondaryTriggerEndOutsideEvent = new Event<InteractorEvent>()

  private onInteractorSecondaryTriggerStartEvent = new Event<InteractorEvent>()
  private onInteractorSecondaryTriggerEndEvent = new Event<InteractorEvent>()
  private onInteractorSecondaryTriggerEndOutsideEvent = new Event<InteractorEvent>()

  private onSecondaryTriggerCanceledEvent = new Event<InteractorEvent>()
  ...
  /**
   * Called whenever the interactable enters the secondary triggered state.
   * This is the gate event for the hold-based secondary trigger: the hold
   * timer only starts if at least one listener is registered for this event
   * on the target Interactable or any of its ancestors.
   */
  onSecondaryTriggerStart = this.onSecondaryTriggerStartEvent.publicApi()

  /**
   * Called whenever an interactor secondary-triggers this interactable.
   */
  onInteractorSecondaryTriggerStart = this.onInteractorSecondaryTriggerStartEvent.publicApi()

  /**
   * Called whenever an interactor continues to secondary-trigger this interactable.
   */
  onSecondaryTriggerUpdate = this.onSecondaryTriggerUpdateEvent.publicApi()

  /**
   * Called whenever the interactable exits the secondary triggered state while the interactor is hovering it.
   */
  onSecondaryTriggerEnd = this.onSecondaryTriggerEndEvent.publicApi()

  /**
   * Called whenever the interactable exits the secondary triggered state while the interactor is not hovering it.
   */
  onSecondaryTriggerEndOutside = this.onSecondaryTriggerEndOutsideEvent.publicApi()

  /**
   * Called whenever an interactor is no longer secondary-triggering the interactable while hovering it.
   */
  onInteractorSecondaryTriggerEnd = this.onInteractorSecondaryTriggerEndEvent.publicApi()

  /**
   * Called whenever an interactor is no longer secondary-triggering the interactable while not hovering it.
   */
  onInteractorSecondaryTriggerEndOutside = this.onInteractorSecondaryTriggerEndOutsideEvent.publicApi()

  /**
   * Called whenever an interactor is lost and was in a secondary trigger event with this interactable.
   */
  onSecondaryTriggerCanceled = this.onSecondaryTriggerCanceledEvent.publicApi()

  /**
   * Per-Interactable override for the secondary trigger plane-distance threshold (in cm).
   * This is the maximum distance on the Interactable's plane from the initial hit point
   * that the ray intersection can drift before the hold timer is aborted.
   * When null, the interactor's global threshold is used.
   */
  secondaryTriggerPlaneDistanceThreshold: number | null = null
...
- All: Supports all targeting modes (Direct, Indirect, and Poke)."
  ...
  /**
   * Determines the targeting priority of this Interactable.
   * Higher-priority interactables are preferred over closer, lower-priority ones
   * during target selection. Set programmatically (not exposed in the Inspector).
   */
  targetingPriority: TargetingPriority = TargetingPriority.Normal
...
filtered pinch events, this Interactable will also use filtered pinch events."
  ...
  /**
   * Notifies the interactable that it is entering secondary trigger state
   * @param eventArgs - event parameters, with omitted interactable
   */
  secondaryTriggerStart = (eventArgs: InteractableEventArgs): void => {
    this.eventHandler.secondaryTriggerStart(eventArgs)
  }

  /**
   * Notifies the interactable that it is still in a secondary triggering state
   * @param eventArgs - event parameters, with omitted interactable
   */
  secondaryTriggerUpdate = (eventArgs: InteractableEventArgs): void => {
    this.eventHandler.secondaryTriggerUpdate(eventArgs)
  }

  /**
   * Notifies the interactable that it is exiting secondary trigger state while the interactor is hovering it
   * @param eventArgs - event parameters, with omitted interactable
   */
  secondaryTriggerEnd = (eventArgs: InteractableEventArgs): void => {
    this.eventHandler.secondaryTriggerEnd(eventArgs)
  }

  /**
   * Notifies the interactable that it is exiting secondary trigger state while the interactor is not hovering it
   * @param eventArgs - event parameters, with omitted interactable
   */
  secondaryTriggerEndOutside = (eventArgs: InteractableEventArgs): void => {
    this.eventHandler.secondaryTriggerEndOutside(eventArgs)
  }

  /**
   * Notifies the interactable that a secondary trigger was cancelled
   * @param eventArgs - event parameters, with omitted interactable
   */
  secondaryTriggerCanceled = (eventArgs: InteractableEventArgs): void => {
    this.eventHandler.secondaryTriggerCanceled(eventArgs)
  }
  ...
  /**
   * Interactors that are secondary-triggering this interactable
   */
  get secondaryTriggeringInteractor(): InteractorInputType {
    return this.eventHandler.secondaryTriggeringInteractor
  }
  ...
  public isDescendantOf(interactable: Interactable) {
    return this.sceneObject.isDescendantOf(interactable.sceneObject)
  }

  private enableColliders(enable: boolean) {
    // `colliders` is a public field populated by external sources (editor @input wiring,
    // UIKit Element subclasses, and SIK's own InteractionManager auto-discovery). Any of
    // those can leave a stale entry when its collider's SceneObject is destroyed without
    // unwiring. Guard at this iteration boundary to match the pattern UIKit's own
    // Element.disableManagedComponents uses, and to keep enable/disable cascades robust
    // when an external caller's lifecycle gets ahead of ours.
  ...
  private createEventHandler(): InteractableEventHandler<InteractableEventArgs> {
    return new InteractableEventHandler<InteractableEventArgs>({
      ...
      // Local secondary trigger events
      onSecondaryTriggerStart: (args) => {
        this.onSecondaryTriggerStartEvent.invoke({...args, interactable: this})
        this.log.v("InteractionEvent : " + "On Secondary Trigger Start Event")
      },
      onInteractorSecondaryTriggerStart: (args) => {
        this.onInteractorSecondaryTriggerStartEvent.invoke({...args, interactable: this})
        this.log.v("InteractionEvent : " + "On Interactor Secondary Trigger Start Event")
      },
      onSecondaryTriggerUpdate: (args) => {
        this.onSecondaryTriggerUpdateEvent.invoke({...args, interactable: this})
      },
      onInteractorSecondaryTriggerEnd: (args) => {
        this.onInteractorSecondaryTriggerEndEvent.invoke({...args, interactable: this})
        this.log.v("InteractionEvent : " + "On Interactor Secondary Trigger End Event")
      },
      onSecondaryTriggerEnd: (args) => {
        this.onSecondaryTriggerEndEvent.invoke({...args, interactable: this})
        this.log.v("InteractionEvent : " + "On Secondary Trigger End Event")
      },
      onInteractorSecondaryTriggerEndOutside: (args) => {
        this.onInteractorSecondaryTriggerEndOutsideEvent.invoke({...args, interactable: this})
        this.log.v("InteractionEvent : " + "On Interactor Secondary Trigger End Outside Event")
      },
      onSecondaryTriggerEndOutside: (args) => {
        this.onSecondaryTriggerEndOutsideEvent.invoke({...args, interactable: this})
        this.log.v("InteractionEvent : " + "On Secondary Trigger End Outside Event")
      },
      onSecondaryTriggerCanceled: (args) => {
        this.onSecondaryTriggerCanceledEvent.invoke({...args, interactable: this})
        this.log.v("InteractionEvent : " + "On Secondary Trigger Canceled Event")
      }
    })
  }
}
