...
interface CachedColliderData {
  ...
  // Frame number when this entry was last validated — skip re-validation within the same frame
  lastValidatedFrame: number
  // Cached Transform reference to avoid repeated getSceneObject().getTransform() native calls
...
export class ColliderUtils {
  private static _updateDispatcher: {frameCount: number} | null = null
  private static getFrameCount(): number {
    if (!this._updateDispatcher) {
      this._updateDispatcher = LensConfig.getInstance().updateDispatcher
    }
    return this._updateDispatcher.frameCount
  }
  ...
  private static getOrCreateCacheEntry(collider: ColliderComponent): CachedColliderData | undefined {
    ...
    if (cachedData?.cachedTransform) {
      transform = cachedData.cachedTransform
    } else {
    ...
    if (cachedData && cachedData.lastValidatedFrame !== currentFrame) {
      if (!this.isCacheEntryValid(collider, cachedData, transform)) {
        this.aabbCache.delete(collider)
        cachedData = undefined
      } else {
        cachedData.lastValidatedFrame = currentFrame
      }
    }
    ...
    if (!cachedData) {
      ...
      cachedData.lastValidatedFrame = currentFrame
      cachedData.cachedTransform = transform
    } else {
      if (!cachedData.cachedTransform) {
        cachedData.cachedTransform = transform
      }
  ...
  private static findRenderMeshVisualInHierarchy(sceneObject: SceneObject): RenderMeshVisual | null {
    ...
    return sceneObject.getComponentInDescendants("Component.RenderMeshVisual", false, false) as RenderMeshVisual | null
  }
