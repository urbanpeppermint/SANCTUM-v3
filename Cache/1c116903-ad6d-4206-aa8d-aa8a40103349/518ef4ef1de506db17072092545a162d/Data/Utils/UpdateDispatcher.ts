...
/**
 * When true, tryLogDebugInfo() emits pipe-delimited SIK_PERF| lines instead of
 * human-readable tables. Intended for CI log parsing; toggled via sed in the CI
 * prep script. Keep false for local development/debugging.
 */
const MACHINE_READABLE_OUTPUT = false
...
class UpdateDispatcherDebug extends UpdateDispatcher {
  ...
  private logMachineReadable(): void {
    const frameCount = this.frameStats.frameCount
    if (frameCount > 0) {
      const avgFrameTimeMs = (this.frameStats.totalFrameTime / frameCount) * 1000
      const worstFrameTimeMs = this.frameStats.maxFrameTime * 1000

      this.frameStats.sessionMaxFrameTime = Math.max(this.frameStats.sessionMaxFrameTime, this.frameStats.maxFrameTime)
      const sessionWorstMs = this.frameStats.sessionMaxFrameTime * 1000

      print(
        `SIK_PERF|FRAME_SUMMARY|${frameCount}|${avgFrameTimeMs.toFixed(4)}|${worstFrameTimeMs.toFixed(4)}|${sessionWorstMs.toFixed(4)}`
      )

      this.frameStats.frameCount = 0
      this.frameStats.totalFrameTime = 0
      this.frameStats.maxFrameTime = 0
      this.frameStats.lastResetTime = getTime()
    }

    print(`SIK_PERF|POOL|Update|${this.updateEventPool.inUse}|${this.updateEventPool.capacity}`)
    print(`SIK_PERF|POOL|LateUpdate|${this.lateUpdateEventPool.inUse}|${this.lateUpdateEventPool.capacity}`)
    print(`SIK_PERF|POOL|Delayed|${this.delayedEventPool.inUse}|${this.delayedEventPool.capacity}`)
    print(`SIK_PERF|POOL|PriorityChange|${this.priorityChangePool.inUse}|${this.priorityChangePool.capacity}`)

    this.logEventPoolMachineReadable("Update", this.updateEventTimings)
    this.logEventPoolMachineReadable("LateUpdate", this.lateUpdateEventTimings)
    this.logEventPoolMachineReadable("Delayed", this.delayedEventTimings)
  }

  private logEventPoolMachineReadable(poolName: string, timings: Map<string, EventTiming>): void {
    timings.forEach((timing, name) => {
      if (timing.intervalTotalCount === 0) {
        return
      }

      const calls = timing.intervalTotalCount
      const totalMs = timing.intervalTotalTime * 1000
      const avgMs = totalMs / calls
      const maxMs = timing.intervalMaxTime * 1000
      const avgPerFrameMs = timing.intervalFramesActive > 0
        ? (timing.intervalTotalTime / timing.intervalFramesActive) * 1000
        : 0
      const maxPerFrameMs = timing.intervalMaxFrameTime * 1000

      print(
        `SIK_PERF|EVENT|${poolName}|${name}|${calls}|${totalMs.toFixed(4)}|${avgMs.toFixed(4)}|${maxMs.toFixed(4)}|${avgPerFrameMs.toFixed(4)}|${maxPerFrameMs.toFixed(4)}`
      )
