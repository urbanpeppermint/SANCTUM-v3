...
/** Flat plane-distance threshold (cm) used for all non-far-field targeting modes. */
const SECONDARY_TRIGGER_NON_FAR_FIELD_THRESHOLD_CM = 1.5
...
hand's palm. Useful for hand-mounted UI interactions."
  ...
  // Scratch vectors for updateNearestPlane to avoid per-frame/per-iteration allocations
  private readonly _scratchRayEnd = new vec3(0, 0, 0)
  private readonly _scratchNegNormal = new vec3(0, 0, 0)

  // Reusable buffer for merged interaction planes (avoids array spread allocation)
  private readonly _mergedPlanesBuffer: InteractionPlane[] = []
  ...
  private cachedIsActiveFn!: () => boolean
  private cachedIsTargetingFn!: () => boolean
  private cachedDirectionFn!: () => vec3 | null
  private cachedStartPointFn!: () => vec3 | null
  private cachedFieldModeTransitionInfoFn!: () => FieldModeTransitionInfo
  private cachedHoveredInteractablesFn!: () => Interactable[]
  ...
  onAwake(): void {
    ...
    // Initialize cached function for isActive
    this.cachedIsActiveFn = this.frameCache.wrapMethod(
      `HandInteractor_${this.handType}_isActive`,
      this,
      this.computeIsActive
    )

    this.cachedIsTargetingFn = this.frameCache.wrap(
      `HandInteractor_${this.handType}_isTargeting`,
      () => this._hand?.isInTargetingPose() ?? false
    )

    this.cachedDirectionFn = this.frameCache.wrap(`HandInteractor_${this.handType}_direction`, () =>
      this.computeDirection()
    )

    this.cachedStartPointFn = this.frameCache.wrap(
      `HandInteractor_${this.handType}_startPoint`,
      () => this.activeTargetProvider?.startPoint ?? null
    )

    this.cachedFieldModeTransitionInfoFn = this.frameCache.wrapMethod(
      `HandInteractor_${this.handType}_fieldModeTransitionInfo`,
      this,
      this.computeFieldModeTransitionInfo
    )

    this.cachedHoveredInteractablesFn = this.frameCache.wrap(
      `HandInteractor_${this.handType}_hoveredInteractables`,
      () => Array.from(this.activeTargetProvider.currentInteractableSet)
    )
  ...
  get startPoint(): vec3 | null {
    return this.cachedStartPointFn()?.clone() ?? null
  }
  ...
  get direction(): vec3 | null {
    return this.cachedDirectionFn()?.clone() ?? null
  }

  private computeDirection(): vec3 | null {
  ...
  get orientation(): quat | null {
    return this._hand?.getPinchDirection() ?? null
  }
  ...
  get interactionStrength(): number | null {
    ...
    return this._hand?.getPinchStrength() ?? null
  }
  ...
  get hoveredInteractables(): Interactable[] {
    return this.cachedHoveredInteractablesFn()
  }
  ...
  override updateState(): void {
    ...
    this.processSecondaryTriggerEvents()
    this.drawHoldTimerDebug()
  }
  ...
  override get planecastDragVector(): vec3 | null {
    ...
    if (this._hand === undefined) return vec3.zero()
    return this._hand.isRecentlyFound() ? vec3.zero() : this.planecastDragProvider.currentDragVector
  }
  ...
  override get currentDragVector(): vec3 | null {
    // If the hand has been recently found, return vec3.zero() to allow time to determine if pinch is sustained.
    if (this._hand === undefined) return vec3.zero()
    return this._hand.isRecentlyFound() ? vec3.zero() : this._currentDragVector
  }
  ...
  override get planecastPoint(): vec3 | null {
    if (this.activeTargetProvider === this.indirectTargetProvider) {
      ...
      return this.positionPlaneIntersection(this.currentInteractable, this._hand.indexTip.position)
    }
  ...
  /**
   * Indirect-provider ray origin, exposed for cursor positioning during poke.
   * The indirect provider continues updating each frame regardless of which provider
   * is active, so its ray data stays valid throughout poke interactions.
   */
  get indirectStartPoint(): vec3 | null {
    return this.indirectTargetProvider?.startPoint ?? null
  }

  /**
   * Indirect-provider ray direction, exposed for cursor positioning during poke.
   */
  get indirectDirection(): vec3 | null {
   ...
   * Use this API in HandRayProvider and CursorViewModel to synchronize ray switching and cursor fading.
   *
  ...
  get fieldModeTransitionInfo(): FieldModeTransitionInfo {
    return this.cachedFieldModeTransitionInfoFn()
  }

  private computeFieldModeTransitionInfo(): FieldModeTransitionInfo {
    if (!this._transition.isActive) {
      return {
    ...
    // The cursor is VISIBLE in FarField, NearField, and Direct.
    // The cursor is HIDDEN in BehindNearField only.
    //
    // Transition patterns:
    // - Far↔Near: Full fade-out/fade-in to hide position jump when ray switches
    // - Near↔Direct: Instant (no animation — cursor stays visible)
    // - Near↔Behind / Behind↔Direct: source or target hidden, single fade
    const sourceIsCursorHidden = this._transition.fromMode === FieldTargetingMode.BehindNearField
    const targetIsCursorHidden = this._transition.toMode === FieldTargetingMode.BehindNearField
    ...
    if (sourceIsCursorHidden && targetIsCursorHidden) {
      ...
      const fadeInElapsed = Math.max(0, elapsed - MODE_TRANSITION_FADE_OUT_DURATION_SEC)
  ...
  isTargeting(): boolean {
    return this.cachedIsTargetingFn()
  }

  /**
   * Returns true if the hand interactor and the hand it is associated with are both enabled.
   * Cached per frame via FrameCache since the result cannot change within a single frame.
   */
  isActive(): boolean {
    return this.cachedIsActiveFn()
  }
  ...
  private computeIsActive(): boolean {
    return (
      ...
      !this._hand.isPhoneInHand &&
      this.sceneObject.isEnabledInHierarchy
    )
  }
  ...
  isTracking(): boolean {
    ...
    return this._hand.enabled && this._hand.isTracked()
  }
  ...
  private updateTarget(): void {
    ...
    } else if (this.activeTargetProvider === this.indirectTargetProvider && this._hand?.isPinching()) {
  ...
  private updatePinchFilter() {
    ...
    if (this.currentInteractable === null) {
      this._hand.useFilteredPinch = false
      return
    }

    let useFilteredPinch = this.currentInteractable.useFilteredPinch

    const ancestorInteractables = this.currentInteractable.sceneObject.getComponentsInAncestors(
      Interactable.getTypeName(),
      false,
      false
    ) as Interactable[]
    for (let i = 0; i < ancestorInteractables.length; i++) {
      useFilteredPinch = useFilteredPinch || ancestorInteractables[i].useFilteredPinch
    }

    this._hand.useFilteredPinch = useFilteredPinch
  }
  ...
  private selectProviderBasedOnPriority(): void {
    ...
    if (this._hand.targetingData?.intendsToTarget) {
  ...
  private needsAnimatedTransition(fromMode: FieldTargetingMode, toMode: FieldTargetingMode): boolean {
    ...
    // Near ↔ Behind: Cursor hidden in both transitions
    if (
  ...
  private setFieldTargetingMode(newMode: FieldTargetingMode): void {
    ...
    // Every code path below mutates _transition state and/or _fieldTargetingMode, both
    // of which feed computeFieldModeTransitionInfo(). Invalidate the FrameCache wrap so
    // any later reads in this same frame (HandRayProvider.getRaycastInfo,
    // CursorViewModel.onUpdate) recompute against the new state.
    //
    // Without this, getEffectiveInteractionPlane()'s "transitioning && !shouldUseNewMode"
    // check uses the stale pre-mutation cache populated earlier by updateNearestPlane,
    // skipping the cached-plane fallback and producing a one-frame "FF flash" on NF exit.
    ;(this.cachedFieldModeTransitionInfoFn as any).clearCache()
  ...
  /**
   * Returns a flat threshold for non-far-field modes (poke, direct pinch,
   * near-field) where distance-based scaling is inappropriate because the
   * interactor is physically close to the target. Far-field interactions
   * delegate to the base implementation which scales with distance.
   */
  protected override getEffectiveSecondaryTriggerThreshold(interactorDistance: number): number {
    if (this._fieldTargetingMode !== FieldTargetingMode.FarField) {
      return SECONDARY_TRIGGER_NON_FAR_FIELD_THRESHOLD_CM
    }
    return super.getEffectiveSecondaryTriggerThreshold(interactorDistance)
  }

  /**
   * For physical interactions (poke / direct pinch), projects the fingertip
   * position (endPoint) onto the hold plane rather than using ray-plane
   * intersection. The PhysicalInteractionProvider's ray (upper joint →
   * fingertip) is nearly co-planar with the surface, making
   * rayPlaneIntersection unreliable or offset.
   *
   * For all other modes (far-field, near-field pinch) the active target
   * provider is the IndirectTargetProvider whose ray is a proper pointing
   * ray, so we delegate to the base rayPlaneIntersection.
   */
  protected override getCurrentHoldPlanePosition(): vec3 | null {
    if (this.activeTargetProvider !== this.physicalInteractionProvider) {
      return super.getCurrentHoldPlanePosition()
    }
    const fingerPos = this.endPoint
    if (fingerPos === null) {
      return null
    }
    return this.projectPointOntoHoldPlane(fingerPos)
  }
  ...
  private computePreventTargetUpdate(): boolean {
    return (this._hand !== undefined && (this._hand.isPinching() || this.isPoking())) || this.isSecondaryTriggering
  }
  ...
  private updateCachedIndexProjection(): void {
    ...
    const indexPoint = this._hand.indexTip.position
    if (indexPoint === null) {
      this._cachedIndexProjection = null
      return
    }
  ...
  private updateNearestPlane(): FieldTargetingMode {
    ...
    let interactionPlanes: InteractionPlane[] = colliderPlanes
    if (effectivePlaneForHysteresis !== null && !colliderPlanes.includes(effectivePlaneForHysteresis)) {
      this._mergedPlanesBuffer.length = 0
      for (let i = 0; i < colliderPlanes.length; i++) {
        this._mergedPlanesBuffer.push(colliderPlanes[i])
      }
      this._mergedPlanesBuffer.push(effectivePlaneForHysteresis)
      interactionPlanes = this._mergedPlanesBuffer
    }

    let nearestPlane: InteractionPlane | null = null
    let nearestDistance = Number.POSITIVE_INFINITY

    const planeRaycastLocus = this._hand.indexTip.position
    ...
    if (farFieldRay !== null) {
      this._scratchRayEnd.copyFrom(farFieldRay.direction)
      this._scratchRayEnd.uniformScaleInPlace(500)
      this._scratchRayEnd.addInPlace(farFieldRay.locus)
      rayEnd = this._scratchRayEnd
    }
    ...
    for (const interactionPlane of interactionPlanes) {
      ...
      this._scratchNegNormal.copyFrom(normal)
      this._scratchNegNormal.uniformScaleInPlace(-1)
      const isTowardPlane =
        farFieldRay !== null &&
        farFieldRay.direction.angleTo(this._scratchNegNormal) < NEAR_FIELD_ANGLE_THRESHOLD_RADIAN
