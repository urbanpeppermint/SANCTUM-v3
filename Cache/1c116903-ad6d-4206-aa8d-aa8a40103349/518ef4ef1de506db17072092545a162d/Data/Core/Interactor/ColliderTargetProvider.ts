...
export abstract class ColliderTargetProvider extends TargetProvider {
  ...
  private readonly _hitsBuffer: RayCastHit[] = []
  ...
  protected onColliderOverlapStay(event: OverlapEnterEventArgs, allowOutOfFovInteraction = false): void {
    ...
    const overlaps = event.currentOverlaps
    this._hitsBuffer.length = 0

    for (let i = 0; i < overlaps.length; i++) {
      try {
        const overlap = overlaps[i]
        this._hitsBuffer.push({
          collider: overlap.collider,
          distance: overlap.collider.getTransform().getWorldPosition().distance(ep),
          normal: vec3.zero(),
          position: ep,
          skipRemaining: false,
          t: 0,
  ...
  protected updateInteractionPlanesFromOverlap(overlaps: Overlap[]): void {
    ...
    for (const overlap of overlaps) {
      ...
      const plane = im.getInteractionPlaneByCollider(overlap.collider)
  ...
  protected removeInteractionPlaneFromOverlap(overlap: Overlap): void {
    // Map only contains plane.collider → plane entries, so child colliders are implicitly excluded.
    const plane = InteractionManager.getInstance().getInteractionPlaneByCollider(overlap.collider)
    if (plane !== null) {
      const index = this.currentInteractionPlanes.indexOf(plane)
