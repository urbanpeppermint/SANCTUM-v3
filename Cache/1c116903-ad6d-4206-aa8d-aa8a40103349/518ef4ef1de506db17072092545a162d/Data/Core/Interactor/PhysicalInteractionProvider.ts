...
export class PhysicalInteractionProvider extends TargetProvider {
  ...
  private _currentInteractionPlanesArray: InteractionPlane[] = []
  ...
  private _interactableSetDirty = true

  // Reusable buffer for collect-then-delete pattern on invalidPokeInteractables
  private _invalidPokeRemovalBuffer: Interactable[] = []

  // Scratch vectors for poke start/end point and midpoint calculations — avoid per-frame allocations
  private readonly _scratchPokeA = new vec3(0, 0, 0)
  private readonly _scratchPokeStart = new vec3(0, 0, 0)
  private readonly _scratchPokeEnd = new vec3(0, 0, 0)
  private readonly _scratchMidpoint = new vec3(0, 0, 0)
  private readonly _scratchVelocity = new vec3(0, 0, 0)

  // Per-frame cached versions of poke start/end point.
  // _scratchPokeStart and _scratchPokeEnd are dedicated (not shared with other methods),
  // so the cached reference remains valid for the entire frame.
  private cachedGetPokeStartPoint: () => vec3
  private cachedGetPokeEndPoint: () => vec3
  ...
  constructor(
    ...
    const frameCache = FrameCache.getInstance()
    const hand = this.config.handType
    this.cachedGetPokeStartPoint = frameCache.wrapMethod(`PIP_${hand}_pokeStartPoint`, this, this.getPokeStartPoint)
    this.cachedGetPokeEndPoint = frameCache.wrapMethod(`PIP_${hand}_pokeEndPoint`, this, this.getPokeEndPoint)
  ...
  get currentInteractionPlanes(): InteractionPlane[] {
    return this._currentInteractionPlanesArray
  }
  ...
  clearInteractionPlane(plane: InteractionPlane): void {
    if (this._currentInteractionPlanes.delete(plane)) {
      const idx = this._currentInteractionPlanesArray.indexOf(plane)
      if (idx !== -1) {
        this._currentInteractionPlanesArray.splice(idx, 1)
      }
    }
  }
   ...
   */
  get startPoint(): vec3 {
    return this.cachedGetPokeStartPoint().clone()
  }

  /** @inheritdoc */
  get endPoint(): vec3 {
    return this.cachedGetPokeEndPoint().clone()
  }
  ...
  private calculatePokeInteractionStrength(): number {
    ...
    // Use current fingertip position - this moves smoothly as you poke
    const currentFingerTipPosition = this.cachedGetPokeEndPoint()
  ...
  reset(): void {
    ...
    this._currentInteractionPlanesArray.length = 0
    ...
    this._interactableSetDirty = true
  ...
  private updateIndexFingerDetection(): void {
    ...
    const startPoint = this.cachedGetPokeStartPoint()
    const endPoint = this.cachedGetPokeEndPoint()

    // Bistability: Use larger radius when actively interacting to maintain contact through hand tracking noise.
    // Includes Pinch because a direct pinch can evolve from a poke and must maintain the same contact zone.
    const radius =
      this._currentTrigger !== InteractorTriggerType.None ? FINGERTIP_MAINTAIN_RADIUS : FINGERTIP_SPHERECAST_RADIUS
    ...
    this.probe.sphereCastAll(radius, startPoint, endPoint, (hits: RayCastHit[]) => {
      ...
      // Collect first, then delete to avoid allocation from Array.from()
      this._invalidPokeRemovalBuffer.length = 0
      for (const invalidInteractable of this.invalidPokeInteractables) {
        if (!this.indexFingerTouchedInteractables.has(invalidInteractable)) {
          this._invalidPokeRemovalBuffer.push(invalidInteractable)
        }
      }
      for (let i = 0; i < this._invalidPokeRemovalBuffer.length; i++) {
        const removed = this._invalidPokeRemovalBuffer[i]
        this.invalidPokeInteractables.delete(removed)
        this.log.d(`Removed ${removed.sceneObject.name} from invalidPokeInteractables (no longer touching)`)
      }
  ...
  private updatePokeValidation(hits: RayCastHit[]): void {
    ...
    this._pokeDepth = closestHit.position.distance(this.cachedGetPokeEndPoint())
   ...
   * Inherited from PokeTargetProvider logic.
   *
   * @returns a scratch vector — do not store the reference. Use immediately or clone.
   */
  private getPokeStartPoint(): vec3 {
    ...
    // Extend the collider length when actively interacting (Poke or Pinch that evolved from Poke).
    ...
    const isActivelyInteracting = this._currentTrigger !== InteractorTriggerType.None
    const indexUpperJointPos = this.hand.indexUpperJoint.position
    const indexTipUp = this.hand.indexTip.up

    if (isActivelyInteracting || !this._pokeIsValid) {
      // Extend, but avoid changing the original raycast angle
      const indexTipPos = this.hand.indexTip.position
      const indexKnucklePos = this.hand.indexKnuckle.position
      const tipToKnuckleDistance = indexTipPos.distance(indexKnucklePos)

      // tipToUpperJointDir = normalize(indexUpperJointPos - indexTipPos)
      this._scratchPokeA.copyFrom(indexUpperJointPos)
      this._scratchPokeA.subInPlace(indexTipPos)
      this._scratchPokeA.normalizeInPlace()

      // result = indexTipPos + indexTipUp * upOffset + tipToUpperJointDir * tipToKnuckleDistance
      this._scratchPokeStart.copyFrom(indexTipUp)
      this._scratchPokeStart.uniformScaleInPlace(upOffset)
      this._scratchPokeStart.addInPlace(indexTipPos)
      this._scratchPokeA.uniformScaleInPlace(tipToKnuckleDistance)
      this._scratchPokeStart.addInPlace(this._scratchPokeA)
      return this._scratchPokeStart
    } else {
      // result = indexUpperJointPos + indexTipUp * upOffset
      this._scratchPokeStart.copyFrom(indexTipUp)
      this._scratchPokeStart.uniformScaleInPlace(upOffset)
      this._scratchPokeStart.addInPlace(indexUpperJointPos)
      return this._scratchPokeStart
    }
  }
  ...
  private getPokeEndPoint(): vec3 {
    ...
    // Evaluate startPoint before mutating _scratchPokeA — cachedGetPokeStartPoint()
    // may trigger getPokeStartPoint() which also uses _scratchPokeA internally.
    const startPoint = this.cachedGetPokeStartPoint()

    // indexTipBack = normalize(indexTipPos - startPoint)
    this._scratchPokeA.copyFrom(indexTipPos)
    this._scratchPokeA.subInPlace(startPoint)
    this._scratchPokeA.normalizeInPlace()

    // result = indexTipPos + indexTipUp * upOffset - indexTipBack * (radius - forwardOffset)
    this._scratchPokeEnd.copyFrom(indexTipUp)
    this._scratchPokeEnd.uniformScaleInPlace(upOffset)
    this._scratchPokeEnd.addInPlace(indexTipPos)
    this._scratchPokeA.uniformScaleInPlace(FINGERTIP_SPHERECAST_RADIUS - forwardOffset)
    this._scratchPokeEnd.subInPlace(this._scratchPokeA)
    return this._scratchPokeEnd
  }
  ...
  private isValidPokeDirection(interactable: Interactable): boolean {
    ...
    this._scratchVelocity.copyFrom(velocity)
    this._scratchVelocity.normalizeInPlace()
    const normalizedVelocity = this._scratchVelocity
  ...
  private addInteractionPlaneFromOverlap(overlap: Overlap): void {
    // Map only contains plane.collider → plane entries, so child colliders are implicitly excluded.
    const plane = this.interactionManager.getInteractionPlaneByCollider(overlap.collider)
    if (plane !== null && !this._currentInteractionPlanes.has(plane)) {
      this._currentInteractionPlanes.add(plane)
      this._currentInteractionPlanesArray.push(plane)
    }
  }
  ...
  private createInteractableHitInfo(interactable: Interactable, targetMode: TargetingMode): InteractableHitInfo {
    // Clone the scratch vector — getHandNearbyPosition returns _scratchMidpoint which will
    // be overwritten on the next call. The hit info is stored in _currentInteractableHitInfo
    // and exposed via a public getter, so callers must get a stable snapshot.
    const position = this.getHandNearbyPosition().clone()
    const interactableTransform = interactable.sceneObject.getTransform()
  ...
  private getHandNearbyPosition(): vec3 {
    this._scratchMidpoint.copyFrom(this.hand.indexTip.position)
    this._scratchMidpoint.addInPlace(this.hand.thumbTip.position)
    this._scratchMidpoint.uniformScaleInPlace(0.5)
    return this._scratchMidpoint
  }
  ...
  private checkForNewInteraction(isPinching: boolean): void {
    ...
    // Get best candidate from currentInteractableSet, preferring higher targetingPriority.
    // Among equal priority, the first element wins (insertion order = closest first).
    let bestCandidate: Interactable | null = null
    for (const candidate of this.currentInteractableSet) {
      if (bestCandidate === null || (candidate.targetingPriority ?? 0) > (bestCandidate.targetingPriority ?? 0)) {
  ...
  override get currentInteractableSet(): Set<Interactable> {
    if (this._interactableSetDirty) {
      ...
      this._interactableSetDirty = false
    }
