...
behaviors (pinch/poke) or override distance thresholds for individual interactables."
  ...
  // Cached material passes for the active hand mesh to avoid per-frame getPass() bridge crossings
  private cachedPasses: Pass[] = []

  // Previous-frame visibility flags used to skip updateVisualsEnabledState when nothing changed
  private lastVisGloballyVisible: boolean | null = null
  private lastVisShowFullMesh = false
  private lastVisShowIndexThumbMesh = false
  private lastVisIndexGlow = false
  private lastVisThumbGlow = false
  ...
  set meshType(newMeshType: HandMeshType) {
    ...
    this.cachePassesForActiveMesh()
    this._glowEffectView?.notifyMeshTypeChanged()
  }

  /** Re-caches Pass references for the active mesh. Called on mesh type or material change. */
  private cachePassesForActiveMesh(): void {
    this.cachedPasses.length = 0
    if (!this.activeHandMesh) return
    const mat = this.activeHandMesh.mainMaterial
    if (!mat) return
    const count = mat.getPassCount()
    for (let i = 0; i < count; i++) {
      this.cachedPasses.push(mat.getPass(i))
    }
  }
  ...
  private updateVisualsEnabledState(): void {
    ...
    if (!this.glowEffectView) {
      ...
      // Short-circuit when visibility state is unchanged to skip SceneObject.enabled writes
      if (
        isGloballyVisible === this.lastVisGloballyVisible &&
        isVisible === this.lastVisShowFullMesh
      ) {
        return
      }
      this.lastVisGloballyVisible = isGloballyVisible
      this.lastVisShowFullMesh = isVisible
    ...
    // Short-circuit when all visibility flags match last frame
    if (
      isGloballyVisible === this.lastVisGloballyVisible &&
      showFullMesh === this.lastVisShowFullMesh &&
      showIndexThumbMesh === this.lastVisShowIndexThumbMesh &&
      finalIndexGlowVisibility === this.lastVisIndexGlow &&
      finalThumbGlowVisibility === this.lastVisThumbGlow
    ) {
      return
    }

    this.lastVisGloballyVisible = isGloballyVisible
    this.lastVisShowFullMesh = showFullMesh
    this.lastVisShowIndexThumbMesh = showIndexThumbMesh
    this.lastVisIndexGlow = finalIndexGlowVisibility
    this.lastVisThumbGlow = finalThumbGlowVisibility
  ...
  private updateFrustumCulling(): void {
    ...
    // Derive right/up/forward axes from a single quat read instead of 3 separate
    // vec3 property reads (5 bridge crossings vs 15).
    ...
    const wristRightX = 1 - 2 * (qy * qy + qz * qz)
    const wristRightY = 2 * (qx * qy + qz * qw)
    const wristRightZ = 2 * (qx * qz - qy * qw)

    const wristUpX = 2 * (qx * qy - qz * qw)
    const wristUpY = 1 - 2 * (qx * qx + qz * qz)
    const wristUpZ = 2 * (qy * qz + qx * qw)

    // Lens Studio forward = -Z axis (right-handed), so negate the rotation matrix Z column
    const wristForwardX = -(2 * (qx * qz + qy * qw))
    const wristForwardY = -(2 * (qy * qz - qx * qw))
    const wristForwardZ = -(1 - 2 * (qx * qx + qy * qy))

    // Skip reading non-index/thumb joint positions when they aren't rendered
    ...
    const passes = this.cachedPasses
    const passCount = passes.length

    if (frustumCull.isLeftHand) {
      frustumCullMin.setXYZ(localMin.x, -localMax.z, localMin.y)
      frustumCullMax.setXYZ(localMax.x, -localMin.z, localMax.y)
    } else {
      frustumCullMin.setXYZ(localMin.x, localMin.y, localMin.z)
      frustumCullMax.setXYZ(localMax.x, localMax.y, localMax.z)
    }

    for (let i = 0; i < passCount; i++) {
      passes[i].frustumCullMin = frustumCullMin
      passes[i].frustumCullMax = frustumCullMax
    }
