...
export class GlowEffectViewModel {
  ...
  // Cached to avoid per-frame multi-condition evaluation in updateVisibilityState.
  // Refreshed on mesh type or visual selection change.
  private cachedShouldAlwaysShowMesh: boolean = true
  ...
  constructor(private config: GlowEffectViewModelConfig) {
    ...
    this.cachedShouldAlwaysShowMesh = this.shouldAlwaysShowMesh()
    ...
    this.hand.onHandFound.add(() => {
      ...
      if (this.stateMachine.currentState !== null) {
        this.stateMachine.enterState(this.stateMachine.currentState.name)
      }
    })
  ...
  refreshShouldAlwaysShowMesh(): void {
    this.cachedShouldAlwaysShowMesh = this.shouldAlwaysShowMesh()
  }
  ...
  // Mutates this.pinchProps in-place to avoid per-frame object allocations.
  private updatePinchEffects(
    isPinching: boolean,
    ...
    if (distance === Infinity && !hasIndirectTarget && !isAlwaysOn && this.pinchValidLerpTime === 0) {
      ...
      p.color = finalColor
      p.glowBrightness = 0
      p.glowColor = finalColor
      p.exponent = this.style.pinchExponent
      return
    }
    ...
    p.brightness = MathUtils.lerp(baseBrightness, triggeredBrightness, pinchTriggerFactor)
    p.color = finalColor
    p.glowBrightness = MathUtils.lerp(baseGlowBrightness, triggeredGlowBrightness, pinchTriggerFactor)
    p.glowColor = finalColor
    p.exponent = MathUtils.lerp(this.style.pinchExponent, this.style.pinchExponentTriggered, pinchTriggerFactor)
  }

  // Mutates this.pokeProps in-place to avoid per-frame object allocations.
  private updatePokeEffects(
    isPoking: boolean,
    isPokeValid: boolean,
    distance: number,
    deltaTime: number,
    ...
    if (distance === Infinity && !isAlwaysOn && !this.isPalmTapping) {
      ...
      pk.color = finalColor
      pk.glowBrightness = 0
      pk.glowColor = finalColor
      pk.highlightGradientMaskPosition = this.currentHighlightGradientMaskPosition
      pk.depthFactor = 0
      pk.occludeGradientMaskPosition = this.currentOccludeGradientMaskPosition
      pk.exponent = this.style.pokeExponent
      return
    }
    ...
    pk.brightness = finalPokeBrightness * pokeBrightnessFactor
    pk.color = finalColor
    pk.glowBrightness = finalPokeGlowBrightness * pokeBrightnessFactor
    pk.glowColor = finalColor
    pk.highlightGradientMaskPosition = this.currentHighlightGradientMaskPosition
    pk.depthFactor = pokeDepthFactor
    pk.occludeGradientMaskPosition = this.currentOccludeGradientMaskPosition
    pk.exponent = MathUtils.lerp(this.style.pokeExponent, this.style.pokeExponentTriggered, pokeTriggerFactor)
  }
  ...
  private updateVisibilityState(
    ...
    if (isAlwaysOn || this.cachedShouldAlwaysShowMesh) {
      this.isMeshVisibilityDesired = true
      this.isIndexGlowVisible = true
      this.isThumbGlowVisible = true
    } else {
  ...
  private resetToBaseState(isAlwaysOn: boolean): void {
    ...
    this.pinchProps.brightness = pinchStartBrightness
    this.pinchProps.color = this.style.hoverColor
    this.pinchProps.glowBrightness = pinchStartGlowBrightness
    this.pinchProps.glowColor = this.style.hoverColor
    this.pinchProps.exponent = this.style.pinchExponent

    this.currentHighlightGradientMaskPosition = this.style.pokeHighlightGradientMaskMin
    this.currentOccludeGradientMaskPosition = this.style.pokeOccludeGradientMaskMin

    this.pokeProps.brightness = pokeStartBrightness
    this.pokeProps.color = this.style.hoverColor
    this.pokeProps.glowBrightness = pokeStartGlowBrightness
    this.pokeProps.glowColor = this.style.hoverColor
    this.pokeProps.depthFactor = 0
    this.pokeProps.highlightGradientMaskPosition = this.currentHighlightGradientMaskPosition
    this.pokeProps.occludeGradientMaskPosition = this.currentOccludeGradientMaskPosition
    this.pokeProps.exponent = this.style.pokeExponent
  ...
  private updateLerpTime(
    ...
    if (isTriggered) {
      return Math.min(currentLerpTime + deltaTime, maxDuration)
    } else {
      return Math.max(currentLerpTime - deltaTime, 0)
    }
  }
