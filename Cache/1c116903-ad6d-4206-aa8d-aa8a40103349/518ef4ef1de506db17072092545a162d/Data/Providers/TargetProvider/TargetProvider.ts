...
export default abstract class TargetProvider {
  ...
  // Cached bound function to avoid per-call .bind() allocation in getInteractableHitFromRayCast
  private boundGetInteractableByCollider = this.interactionManager.getInteractableByCollider.bind(
  ...
  static getNearestDeeplyNestedInteractable(hitInfos: InteractableHitInfo[]): InteractableHitInfo | null {
    hitInfos.sort((hitA, hitB) => {
      const priorityDiff =
        (hitB.interactable.targetingPriority ?? 0) - (hitA.interactable.targetingPriority ?? 0)
      if (priorityDiff !== 0) {
        return priorityDiff
      }
    ...
    for (const currentHitInfo of hitInfos) {
      // Self-check needed: multiple colliders on the same SceneObject can produce duplicate hitInfos.
      // Native isDescendantOf does not treat self as descendant (starts from parent).
      if (
        targetHitInfo === null ||
        currentHitInfo.interactable.sceneObject === targetHitInfo.interactable.sceneObject ||
        currentHitInfo.interactable.sceneObject.isDescendantOf(targetHitInfo.interactable.sceneObject)
      ) {
  ...
  protected getInteractableHitFromRayCast(
    ...
    return TargetProvider.getInteractableHitFromRayCast(
      hits,
      this.targetingMode,
      this.boundGetInteractableByCollider,
      offset,
      this.camera,
