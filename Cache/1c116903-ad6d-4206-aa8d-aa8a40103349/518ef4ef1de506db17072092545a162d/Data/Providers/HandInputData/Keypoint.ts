...
export class Keypoint {
  ...
  // Per-frame position cache — avoids redundant getWorldPosition() bridge crossings
  // when the same keypoint's position is read from multiple call sites in one frame.
  private cachedGetPosition: () => vec3
  ...
  constructor(
    ...
    this.cachedGetPosition = FrameCache.getInstance().wrap(
      `Keypoint_${handType}_${name}`,
      () => this.transform.getWorldPosition()
    )
  }

  /**
   * Returns the world position of this keypoint, cached per frame.
   * First access each frame calls getWorldPosition(); subsequent accesses return the cached value.
   */
  get position(): vec3 {
    return this.cachedGetPosition()
  }
