...
export default class IndirectTargetProvider extends TargetProvider {
  ...
  private indirectRayCast(ray: RaycastInfo) {
    const end = ray.direction.uniformScale(this.config.maxRayDistance)
    end.addInPlace(ray.locus)
  ...
  private indirectSphereCast(ray: RaycastInfo, index: number = 0): void {
    ...
    const castOrigin = dir.uniformScale(offset)
    castOrigin.addInPlace(ray.locus)
    const castEnd = dir.uniformScale(this.config.maxRayDistance - offset)
    castEnd.addInPlace(castOrigin)
