 ...
 * Mesh objects rotate around Y axis infinitely, direction changes on button press
...
export class AudioButtonManager extends BaseScriptComponent {
    ...
    // Mesh objects to rotate around Y axis
    @input
    meshObject1: SceneObject;
    @input
    meshObject2: SceneObject;
    @input
    meshObject3: SceneObject;
    @input
    meshObject4: SceneObject;
    @input
    meshObject5: SceneObject;
    @input
    meshObject6: SceneObject;
    @input
    meshObject7: SceneObject;
    ...
    onAwake(): void {
        ...
        // Start the update loop for rotation
        this.startRotationUpdate();
    ...
    private setupArrays(): void {
        ...
        this.meshObjects = [
            this.meshObject1, this.meshObject2, this.meshObject3,
            this.meshObject4, this.meshObject5, this.meshObject6,
            this.meshObject7
        ];
    }

    // Start the per-frame update event for rotating mesh objects
    private startRotationUpdate(): void {
    ...
    // Rotate all assigned mesh objects around Y axis every frame
    private rotateMeshObjects(): void {
        const deltaTime = getDeltaTime();
        const degreesThisFrame = this.rotationSpeed * deltaTime * this.rotationDirection;
        ...
        for (let i = 0; i < this.meshObjects.length; i++) {
            ...
            // Create a delta rotation around Y axis
            const deltaRotation = quat.fromEulerAngles(0, radiansThisFrame, 0);

            // Multiply current rotation by delta rotation
            const newRotation = currentRotation.multiply(deltaRotation);
            transform.setLocalRotation(newRotation);
        }
    }

    // Flip the rotation direction (called on button press)
    private flipRotationDirection(): void {
        this.rotationDirection *= -1;
        print(`Rotation direction changed to: ${this.rotationDirection === 1 ? "Clockwise" : "Counter-Clockwise"}`);

        if (this.debugText) {
            this.debugText.text = `Rotation: ${this.rotationDirection === 1 ? "CW" : "CCW"}`;
        }
    }
    ...
    private validateComponents(): void {
        ...
        for (let i = 0; i < this.meshObjects.length; i++) {
            if (!this.meshObjects[i]) {
                print(`WARNING: Mesh Object ${i + 1} not set`);
            }
        }
            ...
            if (this.buttons[i] && this.audioComponents[i]) {
                ...
                if (interactable) {
                    ...
                    const onTriggerEndCallback = (event: InteractorEvent) => {
                        ...
                        // Flip rotation direction on every button press
                        this.flipRotationDirection();
                    };
    ...
    // Public method to manually set rotation direction (1 or -1)
    public setRotationDirection(direction: number): void {
