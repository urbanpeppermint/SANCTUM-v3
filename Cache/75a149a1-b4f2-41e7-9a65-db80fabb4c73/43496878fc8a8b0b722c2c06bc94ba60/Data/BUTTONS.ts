 ...
 * Rotates designated mesh objects continuously around the Y axis, 
 * reversing direction every time any button is pressed.
...
export class AudioButtonManager extends BaseScriptComponent {
    ...
    // --- ROTATION SYSTEM INPUTS ---
    @ui.separator
    @ui.label("Rotation Settings")
    @input
    objectsToRotate: SceneObject[] = [];
    @input
    rotationSpeed: number = 45.0; // Degrees per second
    ...
    // Rotation direction tracker: 1 = clockwise, -1 = counter-clockwise
    private rotationDirection: number = 1.0; 
    ...
    onAwake(): void {
        ...
        // Set up continuous frame-update listener for infinite rotation
    ...
    private validateComponents(): void {
        ...
        // Check rotation list
        if (this.objectsToRotate.length === 0) {
            print("WARNING: No objects assigned to 'Objects To Rotate' array.");
        }
            ...
            if (this.buttons[i] && this.audioComponents[i]) {
                ...
                if (interactable) {
                    ...
                    const onTriggerEndCallback = (event: InteractorEvent) => {
                        ...
                        this.toggleRotationDirection(); // Reverses spinning direction on any press
                    };
    ...
    // Toggle rotation direction multiplier between 1.0 (clockwise) and -1.0 (counter-clockwise)
    private toggleRotationDirection(): void {
        this.rotationDirection *= -1.0;
        print(`Rotation direction toggled to: ${this.rotationDirection}`);
    }

    // Rotates the registered scene objects around their local Y axis
    private updateRotation(): void {
        if (this.objectsToRotate.length === 0) {
            return;
        }

        // Calculate delta angle in radians based on speed, direction, and frame time
        const dt = getDeltaTime();
        const angleRad = (this.rotationSpeed * this.rotationDirection * dt) * (Math.PI / 180);
        const rotationOffset = quat.angleAxis(angleRad, vec3.up());

        for (let i = 0; i < this.objectsToRotate.length; i++) {
            const obj = this.objectsToRotate[i];
            if (obj) {
                const transform = obj.getTransform();
                const currentRotation = transform.getLocalRotation();
                // Apply rotation offset locally
                transform.setLocalRotation(currentRotation.multiply(rotationOffset));
            }
        }
    }
