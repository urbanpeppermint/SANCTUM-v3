...
exports.OUT_OF_SCOPE_REDIRECT = exports.MANIFESTATION_IMAGE_STYLE = exports.MANIFESTATION_SUGGESTIONS = exports.MANIFESTATION_DESIRES = exports.CHAKRAS = exports.ACUPRESSURE_POINTS = exports.BREATHING_IMAGE_PROMPTS = exports.BREATHING_TOTAL_CYCLES = exports.BREATHING_PHASES = void 0;
exports.buildManifestationImagePrompt = buildManifestationImagePrompt;
exports.buildChakraScopePrompt = buildChakraScopePrompt;
exports.buildChakraScopeCard = buildChakraScopeCard;
exports.buildChakraDisplayText = buildChakraDisplayText;
exports.buildChakraMeditationScript = buildChakraMeditationScript;
exports.BREATHING_PHASES = [
...
exports.BREATHING_TOTAL_CYCLES = 3;
exports.BREATHING_IMAGE_PROMPTS = [
...
exports.ACUPRESSURE_POINTS = [
    {
        name: "He Gu (LI4)",
        location: "In the web between your thumb and index finger",
        instruction: "Using the thumb and index finger of your opposite hand, firmly pinch the fleshy area between the thumb and index finger. Apply steady pressure in a circular motion for 30 seconds. Relieves stress, headaches, and tension.",
        imagePrompt: "Professional minimalist medical illustration on solid black background. White clean line drawing of a human hand, palm facing upward, detailed anatomy. A single bright glowing red circle marks the pressure point in the fleshy web between the thumb and index finger. Three small white circles containing the numerals 1, 2, 3 placed along thin white curved arrows showing a circular massage motion around the red point. High contrast, clean precise anatomical line art, elegant, no words, no labels, no text except the numerals 1 2 3 inside the circles.",
    },
    {
        name: "Nei Guan (PC6)",
        location: "Inner wrist, two finger-widths below the wrist crease",
        instruction: "Turn your palm up. Place three fingers across your wrist from the crease. The point is below your index finger between the two tendons. Press firmly with your thumb for 30 seconds while breathing deeply. Calms anxiety.",
        imagePrompt: "Professional minimalist medical illustration on solid black background. White clean line drawing of a human forearm and wrist, inner side facing up, detailed anatomy showing tendons. A single bright glowing red circle marks the pressure point on the inner wrist two finger-widths below the wrist crease between two tendons. A white arrow from three fingers placed across the wrist shows the measuring position. Three small white circles containing numerals 1, 2, 3 along thin white arrows showing pressing motion. High contrast, clean precise anatomical line art, elegant, no words, no labels, no text except the numerals 1 2 3.",
    },
    {
        name: "Yin Tang (Third Eye)",
        location: "Between the eyebrows, centre of the forehead",
        instruction: "Using your index finger, apply gentle but firm pressure directly between your eyebrows. Press in small circles for 30 seconds. Breathe slowly. Calms the mind and relieves anxiety.",
        imagePrompt: "Professional minimalist medical illustration on solid black background. White clean line drawing of a human face, front view, serene expression, detailed anatomy. A single bright glowing red circle marks the pressure point centered between the eyebrows on the forehead. A white drawn index finger approaches the point. Three small white circles containing numerals 1, 2, 3 along thin white curved arrows showing small circular pressing motion. High contrast, clean precise anatomical line art, elegant, no words, no labels, no text except the numerals 1 2 3.",
    },
    {
        name: "Jian Jing (GB21)",
        location: "Highest point of the shoulder muscle, midway between neck and shoulder edge",
        instruction: "Reach across with your opposite hand and find the midpoint of your shoulder muscle. Press down firmly with your fingers. Hold steady pressure for 30 seconds. Releases shoulder tension and stress.",
        imagePrompt: "Professional minimalist medical illustration on solid black background. White clean line drawing of a human upper body from behind, showing neck and both shoulders, detailed anatomy of trapezius muscle. A single bright glowing red circle marks the pressure point on top of the right shoulder muscle midway between neck and shoulder edge. A white drawn opposite hand reaches across to press the point. Three small white circles containing numerals 1, 2, 3 along thin white arrows showing downward pressing motion. High contrast, clean precise anatomical line art, elegant, no words, no labels, no text except the numerals 1 2 3.",
    },
    {
        name: "Shen Men (HT7)",
        location: "On the wrist crease, pinky side, in the small hollow",
        instruction: "Turn your palm up, find the wrist crease on the pinky side. Feel for a small dip next to the tendon. Apply gentle pressure with your opposite thumb for 30 seconds. The Spirit Gate, deeply calming for emotional stress.",
        imagePrompt: "Professional minimalist medical illustration on solid black background. White clean line drawing of a human hand and wrist, palm facing up, detailed anatomy showing wrist crease and tendons. A single bright glowing red circle marks the pressure point on the wrist crease on the pinky finger side in a small hollow beside the tendon. A white drawn opposite thumb presses the point. Three small white circles containing numerals 1, 2, 3 along thin white arrows showing gentle pressing motion. High contrast, clean precise anatomical line art, elegant, no words, no labels, no text except the numerals 1 2 3.",
    },
];
exports.CHAKRAS = [
    {
...
exports.MANIFESTATION_SUGGESTIONS = exports.MANIFESTATION_DESIRES;
exports.MANIFESTATION_IMAGE_STYLE = "Abstract symbolic wellness art for meditation visualization. NO human figures, NO silhouettes, NO seated meditation poses, NO chakras on a body, NO lotus figures. Use landscape, light, geometry, or nature metaphors that evoke the emotional quality of the intention. Distinct palette per theme. Cinematic depth, soft volumetric glow, painterly clarity, single strong focal symbol. No text, no words, no letters, no UI. 1024x1024.";
const INTENT_VISUAL_METAPHORS = {
...
function metaphorForIntent(intent) {
    const lower = intent.toLowerCase();
    for (const key in INTENT_VISUAL_METAPHORS) {
        if (lower.includes(key))
            return INTENT_VISUAL_METAPHORS[key];
    }
    return ("a unique symbolic landscape of light and color embodying the felt sense of: " + intent +
        " — avoid any human form");
}
function buildManifestationImagePrompt(intent) {
    return (exports.MANIFESTATION_IMAGE_STYLE +
...
exports.OUT_OF_SCOPE_REDIRECT = "Sanctum helps you visualize and embody your intention with calm — not predict the future. Let's refocus on how you want to feel as you move toward this.";
function buildChakraScopePrompt(chakra) {
...
function buildChakraScopeCard(chakra) {
    return (chakra.name +
        " Chakra (" +
        chakra.sanskritName +
        ")\n" +
...
function buildChakraDisplayText(chakra) {
    return (chakra.name +
        " · " +
        chakra.frequencyLabel +
        "\n\n" +
...
function buildChakraMeditationScript(chakra) {
    return ("Settle into stillness... and gently bring your awareness to " +
        chakra.location +
        ".\n\n" +
        "The " +
        chakra.frequencyLabel +
...
//# sourceMappingURL=SanctumData.js.map