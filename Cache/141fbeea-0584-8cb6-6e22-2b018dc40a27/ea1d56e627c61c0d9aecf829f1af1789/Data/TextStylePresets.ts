...
export class TextStylePresets extends BaseScriptComponent {
  ...
  private _ranking: string = "Title1"
  ...
  private updateTextStyle() {
    const thisSettings = RankingDistanceToSettings[this._ranking as Ranking][this._distance as Distance]
    this.textComponent.size = thisSettings.size
    // if this is 5.16 and above
    if (!isNull((this.textComponent as any).weight)) {
      if (thisSettings.weight !== undefined) {
        ;(this.textComponent as any).weight = thisSettings.weight
      }
      this.textComponent.size = this.convert516Size(thisSettings.size)
    }
  }
